...
c-----[--.----+----.----+----.-----------------------------------------]
	...
	integer l,ndf,ndm,nst,isw,nhv,tdof,pdof,lint,i,j,m,j1
      ...
      real*8  xsj,shp(4,nen),dv,sg(4,nen),ul(ndf,nen,*),DeltaFi(3)
      ...
      real*8  pomocna(3,3)
      real*8  Q3T(3,3),Q4T(3,3),Q7T(3,3),Q8T(3,3),Q11T(3,3),Q15T(3,3)
      real*8  normFi3,normFi4,normFi7,normFi8,normFi11,normFi15
      real*8  surfaceload(3)
      real*8  folload3(3),folload4(3),folload7a(3),folload7(3)
      real*8  folload8a(3),folload8(3),folload11(3),folload15(3)
      real*8  forceF1,forceF2,surloadF1(3),surloadF2(3)
      real*8  skewfolload3(3,3),skewfolload4(3,3),skewfolload7(3,3)
      real*8  skewfolload8(3,3),skewfolload11(3,3),skewfolload15(3,3)
      real*8  numstepinput,numsteps
      
c     Output element type

      if(isw.eq.0 .and. ior.lt.0) then
        write(iow,'(A)') 'Elmt  09: 3-D MPC Geom NonLinear,Elastic'
        !if (nen.eq.20) write(iow,'(A)') '20-node hexahedral FE'
        if (nen.eq.27) write(iow,'(A)') '27-node hexahedral FE'
...
c.... set history storage for curvatures + quaternions
      ...
      ngp = 27 !number of gauss integration points
      ...
      hplmax = nh1 !for paraview, nh1 + nh2 + nh3
		...
		if(ior.lt.0) write(*,2001)
          ...
          write(iow,'(A)')    ' NUM LOAD STEPS:'
          write(iow,'(A,E10.3)') '    numstepinput    = ',numstepinput
...
c			Compute Gauss quadrature points and weights
			...
			l=3  !number of int points per direction

			call int3d(l,lint,sg) !sg(4,nen) - array of points and weights
...
!          if(niter.eq.0) then
!             !write (iow,*) 'matrix Q has to be identity'
!             !write (iow,*) 'quaternion defined as {q,q0}={{0,0,0},1}'
!             
!             do i=1,3
!             Q(i,i) = 1.0d0
!             enddo
...
c %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          ...
          do j=1,3
               do i=1,nel
                  ...
                  !write(iow,'(A,E10.3)') '	ul(j,i,3)=',ul(j,i,3)
                  !write(iow,'(A,E10.3)') '	ul(3+j,i,3)=',ul(3+j,i,3)
                  !write(iow,'(A,E10.3)') 'disp ul(j,i,1)=',ul(j,i,1)
                  !write(iow,'(A,E10.3)') 'disp ul(3+j,i,1)=',ul(3+j,i,1)
               enddo !i
          enddo !j
          
              !do i=1,3      
              !    write(iow,*) i
              !    write(iow,'(A,E10.3)') '	DeltaFi(i)=',DeltaFi(i)
              !end do !i
...
c   Recover data from history variables
              ...
              do i=1,4
                  quaOLD(i) = hr (nh1+nn1-1+i) !nh1 - last converged
              enddo !
...
c .........Transpose of the deformation gradient matrix
		...
		do i=1,3 !loop F -> FT
			...
			FT(j,i)=F(i,j)
              ...
              !write (iow,*)
              !write(iow,'(A)') '-------	  Matrix FT	-------'
              !do i=1,3
              !    write (iow,'(E13.6)') (FT(i,j),j=1,3)
              !    write(iow,'(A)') '	new row'
              !end do
...
c .........Form the strain as E = QT.F - I
          ...
          !write (iow,*)
          !write(iow,'(A)') '-------	  Matrix Estrain	-------'
          !    do i=1,3
          !        write (iow,*) (Estrain(i,j),j=1,3)
          !        write(iow,'(A)') '	new row'
          !    end do
...
c     FORM CURVATURE TENSOR
          ...
          enddo !i
              !
              !write (iow,*)
              !write(iow,'(A)') '-------	  curvature K	-------'
              !do i=1,3
              !    write (iow,*) (Knew(i,j),j=1,3)
              !    write(iow,'(A)') '	new row'
              !end do
...
c  -----     TERMS NEEDED IN THE RESIDUAL r[6,nel]   -----

c ->->->->->-> LOOP i- for i-th nodal residual

          do i=1,nen !summation
...
c  -----     TERMS NEEDED IN THE MATERIAL STIFFNESS MATRIX Km[6,6]   -----
...
c .............. Form the term -(mi+ni)*Ni*Nj*[(F.FT)-tr(F.FT).Iden]  =  term18(3,3)
...
c CONTRIBUTION OF THE SURFACE LOADING INTO THE RESIDUAL
      ...
      !write(iow,'(A,E10.3)') '    numsteps        = ',numsteps
          
!c     case 0
!      forceF1 = 0.04222570770954023/numsteps*nstep
!      forceF2 = 0.168902830838161d0/numsteps*nstep      
      
!c     case 1
!      forceF1 = 0.0267142232448d0/numsteps*nstep
!      forceF2 = 0.1068568929792d0/numsteps*nstep
      
!c     case 2
!      forceF1 = 0.01081815652062601d0/numsteps*nstep
!      forceF2 = 0.04327262608250405d0/numsteps*nstep
      
!c     case 3
!      forceF1 = 0.003200481513437032d0/numsteps*nstep
!      forceF2 = 0.01280192605374813d0/numsteps*nstep
	...
	call zeromatrix(Q8T,3,3)
      call zeromatrix(Q11T,3,3)
	call zeromatrix(Q15T,3,3)
      
      call pzero(surloadF1,3)
      call pzero(surloadF2,3)
...
c 		Find the norm of nodal microrotations
	...
	normFi8  = sqrt(ul(4,8,1)**2+ul(5,8,1)**2+ul(6,8,1)**2)
      normFi11 = sqrt(ul(4,11,1)**2+ul(5,11,1)**2+ul(6,11,1)**2)
	normFi15 = sqrt(ul(4,15,1)**2+ul(5,15,1)**2+ul(6,15,1)**2)
...
c       Form orientation matrices NOT TRANSPOSED!!!
	...
	Q11T(1,1)=cos(normFi11)
	Q11T(1,2)=-sin(normFi11)
	Q11T(2,1)=sin(normFi11)
	Q11T(2,2)=cos(normFi11)
      Q11T(3,3)=1.0d0
	
	Q15T(1,1)=cos(normFi15)
	Q15T(1,2)=-sin(normFi15)
	Q15T(2,1)=sin(normFi15)
	Q15T(2,2)=cos(normFi15)
      Q15T(3,3)=1.0d0
         ...
         !
			      !write (iow,*)
         !         write(iow,'(A)') '----Matrix Q11T ----'
         !         do i=1,3
         !             write (iow,'(E13.6)') (Q11T(i,j),j=1,3)
         !             write(iow,'(A)') '	new row'
         !         end do
			      !
			      !write (iow,*)
         !         write(iow,'(A)') '----Matrix Q15T ----'
         !         do i=1,3
         !             write (iow,'(E13.6)') (Q15T(i,j),j=1,3)
         !             write(iow,'(A)') '	new row'
         !         end do
...
c  Contribution of external surface loading to the residual vector
      ...
      !write (iow,*)
      !write(iow,'(A)') '-------	surloadF2 after pzero	-------'
      !         do m=1,3
      !            write (iow,'(E16.5)') surloadF2(m)
      !            write(iow,'(A)') '	new row'
      !end do
      ...
      !write (iow,*)
      !write(iow,'(A)') '-------	surloadF1 after = forceF1	-------'
      !         do m=1,3
      !            write (iow,'(E16.5)') surloadF1(m)
      !            write(iow,'(A)') '	new row'
      !end do
      !
      !write (iow,*)
      !write(iow,'(A)') '-------	surloadF2 after = forceF2	-------'
      !         do m=1,3
      !            write (iow,'(E16.5)') surloadF2(m)
      !            write(iow,'(A)') '	new row'
      !end do
	...
	call matvectmultip(Q8T,-surloadF1,folload8,3,3)
      ...
      call matvectmultip(Q11T,surloadF2,folload11,3,3)
      
      call matvectmultip(Q15T,-surloadF2,folload15,3,3)
      ...
      !            
      !        write (iow,*)
      !         write(iow,'(A)') '-------	folload11	-------'
      !         do m=1,3
      !            write (iow,'(E16.5)') folload11(m)
      !            write(iow,'(A)') '	new row'
      !         end do
      !         
      !        write (iow,*)
      !         write(iow,'(A)') '-------	folload15	-------'
      !         do m=1,3
      !            write (iow,'(E16.5)') folload15(m)
      !            write(iow,'(A)') '	new row'
      !         end do
...
c    Contribute to the element residual
		...
		r(m,3)=r(m,3)+folload3(m)
          ...
          r(m,11)=r(m,11)+folload11(m)
          r(m,15)=r(m,15)+folload15(m)
          enddo !m
...
c    Contribute to the element stiffness matrix
          ...
          call hatmatrixfromvector(folload11,skewfolload11)
          call hatmatrixfromvector(folload15,skewfolload15)
...
c    ....For the 11th node
      ...
      do i=1,3
          do j=1,3
          s(6*fn-6+i,6*fn-3+j)=s(6*fn-6+i,6*fn-3+j)+skewfolload11(i,j)
          enddo !i
      enddo !j
...
c    ....For the 15th node
      ...
      do i=1,3
          do j=1,3
          s(6*fn-6+i,6*fn-3+j)=s(6*fn-6+i,6*fn-3+j)+skewfolload15(i,j)
          enddo !i
      enddo !j
